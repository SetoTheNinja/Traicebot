...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None):
        ...
        self.http.user_agent += ' HelixBot/%s' % BOTVERSION
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        ...
                        'Use %sjoin to make HelixBot join your voice your voice channel' % self.config.command_prefix)
...
#######################################################################################################################
    ...
    async def on_ready(self):
        ...
        log.info('Startup Complete! HelixBot {}\n'.format(BOTVERSION))
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += "Do the credits command to find the other projects this bot was based on"
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        try:
            if server_link:
                ...
                return Response("**Heres The Invite Link!")
    ...
    async def cmd_stream(self, player, channel, author, permissions, song_url):
        ...
        return Response("**Stream will start soon**", delete_after=20)
    ...
    async def cmd_sendall(self, args, leftover_args, channel):
        """
        Usage:
        ...
        for s in self.servers:
            await self.safe_send_message(channel, s, args)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            action_text = 'Stream' if streaming else 'Song'
    ...
    async def cmd_join(self, channel, server, author, voice_channel):
        ...
        if not author.voice_channel:
            ...
            await self.safe_send_message(channel, "Connected!")
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        return Response("***Shuffling Completed!***", delete_after=15)

    async def cmd_tclean(self, player, author):
        """
        Usage:
            {command_prefix}tclean
        Clears the playlist.
        """

        player.playlist.clear()
        return Response('**The playlist has been cleared**', delete_after=20)

    async def cmd_tclean(self, player, author):
        """
        Usage:
            {command_prefix}tclear

        Clears the queue.
        """

        player.playlist.clear()
        return Response('***Queue Cleared***', delete_after=20)
    ...
    async def cmd_forceskip(self, channel, author, user_mentions, player, message):
        ...
        Usage:
            ...
            This is used by admins to forceskip something that should not be playing
        """

        if player.is_stopped:
            raise exceptions.CommandError("Can't Skip! There's nothing playing", expire_in=20)

        if player.is_paused:
            raise exceptions.CommandError("Can't Skip! It's Paused", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("Its Downloading the Song please wait" % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    print("The next song will be played shortly.  Please wait.")
                else:
                    await self.safe_send_message(channel, "HelixBot has stopped responding")
            else:
                await self.safe_send_message(channel, "Something is happening, do the restart to fix this")
    ...
    async def cmd_credits(self, channel, player):
        """
        Usage:
            {command_prefix}credits

        Credits for people and projects that helped complete this one
        """
        await self.safe_send_message(channel, "**Links to projects that complete this one**\nhttps://goo.gl/NkXJLa\nhttps://goo.gl/gnTAoM\nhttps://goo.gl/zyF7qT\n**So far thats all I got**")
        return
    ...
    async def cmd_cclean(self, message, channel, server, author, search_range=50):
        """
        Usage:
            {command_prefix}cclean [range]
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        return Response("**Playlist Dumped**", delete_after=20)
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        return Response("**Ids Sent**", delete_after=20)
    ...
    async def cmd_perms(self, author, channel, server, permissions):
        ...
        return Response("**Perms Sent**", delete_after=20)
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        return Response("**Name Set**", delete_after=20)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        return Response("**NickName Set**", delete_after=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        return Response("**Avatar Set**", delete_after=20)


    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("**Disconnected**", delete_after=20)

    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, "**Please Wait**\n**Restarting**")
        await self.disconnect_all_voice_clients()
    ...
    async def on_server_join(self, server:discord.Server):
        ...
        if not self.user.bot:
            alertmsg = "<@{uid}> Hi I'm a HelixBot please mute me."
